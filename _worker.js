  ...
  async fetch(request, env, ctx) {
    ...
    if (pathParts[1] === "player_api.php") {
      ...
      if (action === 'get_vod_info' && vodId) {
        const redirectUrl = `http://cdn22.cc/player_api.php?username=6705646555&password=60670&get_vods=546&action=get_vod_info&vod_id=${vodId}`;
        return Response.redirect(redirectUrl, 302);
      }

      if (action === 'get_series_info' && seriesId) {
        const redirectUrl = `http://cdn22.cc/player_api.php?username=6705646555&password=60670&get_vods=546&action=get_series_info&series_id=${seriesId}`;
        return Response.redirect(redirectUrl, 302);
      }
